...
func main() {
	...
	chatService := services.NewChatService(connDb, log)
	messageService := services.NewMessageService(connDb, log)
	s := grpc.NewServer()

	reflection.Register(s)

	pbch.RegisterChatServiceServer(s, chatService)
	message.RegisterMessageServiceServer(s, messageService)
